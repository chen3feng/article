...
醒，是再正常不过的事情。这意味着：跨过 `jump_context` 那一刻，`thread_local` 的物理地
址（具体哪个内存槽）变了——之前那个槽是 A 这个 OS 线程的，恢复之后应该用 B 的槽。

C/C++ 标准里没有任何属性可以表达"这个调用可能让我换一个 OS 线程跑"。`returns_twice`
（`setjmp`/`vfork` 用的那个）听起来很贴切，但它的设计前提是"同一个线程上的第二次返回"。
...
三份汇编对照（这里只摘 `jump_context` 前后的关键片段，完整汇编见
[gist](https://gist.github.com/chen3feng/b99e550dc8a9452fbe9b8e88866cbc6e)）：
...
`noinline` 的效果不是"禁止 CSE"，而是**把访问藏进一次真正的函数调用里**。caller 看到的
是一个不透明的 `bl GetCurrentFiberEntity`——按 ABI 规约，函数返回值不能跨调用复用，
**编译器没有机会做 CSE**。

代价是每次访问多一次函数调用——大约 3 条指令的开销（push 帧、call、ret 出来）——对
比一次 fiber 切换的开销（保存恢复整个寄存器组、可能换 OS 线程），是完全可以忽略的。
...
三份 `.s` 文件和最小可编译源码都放在
[gist](https://gist.github.com/chen3feng/b99e550dc8a9452fbe9b8e88866cbc6e) 里：

- Apple Clang macOS arm64
- Clang 重定向到 `aarch64-linux-gnu`
- aarch64-elf GCC

每份只截取了 `Test()` 函数体本身，便于直接对比。
